...
1. ~~**Cotação de múltiplos veículos**~~ — FEITO em 02/09/2026 (brief P: `DoQuotes`, `PresentMany`, plano no início com `plano_padrao`). Buraco restante: CEP inválido respondendo à pergunta do plano repete a pergunta do plano (precisa de `plano_perguntado` em `LeadState`).
   Histórico: (reportado pelo Gabriel em 02/09/2026, teste manual): o lead pede cotação de mais de um carro e o agente
